...
function renderTracklist(){{trackListEl.innerHTML='';playlist.forEach((t,idx)=>{{const row=document.createElement('div');row.className=`track-row ${{idx===currentIndex?'active':''}}`;const img=t.image||'{escaped_img}';row.innerHTML=`<img src="${{img}}" class="track-row-art" alt=""><div class="track-row-info"><span class="track-row-title">${{t.name}}</span><span class="track-row-artist">${{t.artist||'{escaped_site_name}'}}</span></div><span class="track-row-dur">${{formatTime(t.duration||0)}}</span>`;row.onclick=()=>setTrack(idx,true);trackListEl.appendChild(row);}});}}
function setTrack(idx,play=true,offset=0){{if(idx<0||idx>=playlist.length)return;currentIndex=idx;seekOffset=offset;const t=playlist[idx];trackTitleEl.textContent=t.name;trackArtistEl.textContent=t.artist||'{escaped_site_name}';const img=t.image||'{escaped_img}';artImg.src=img;backdrop.style.backgroundImage=`url('${{img}}')`;let base=t.stream_url.startsWith('http')?t.stream_url:window.location.origin+t.stream_url;const join=base.includes('?')?'&':'?';audio.src=`${{base}}${{join}}offset=${{Math.floor(offset)}}&_t=${{Date.now()}}`;audio.load();if(playlist.length>1)history.replaceState(null,'',`#${{t.id||(idx+1)}}`);const dur=getTrackDuration();durationEl.textContent=formatTime(dur);progressBar.style.width=(dur>0?Math.min(100,Math.max(0,(offset/dur)*100)):0)+'%';currentTimeEl.textContent=formatTime(offset);renderTracklist();if('mediaSession'in navigator)navigator.mediaSession.metadata=new MediaMetadata({{title:t.name,artist:t.artist,artwork:[{{src:img}}]}});if(play){{audio.play().catch(e=>{{console.log('Autoplay prevented:',e);const unlock=()=>{{audio.play().catch(()=>{{}});window.removeEventListener('click',unlock);window.removeEventListener('keydown',unlock);window.removeEventListener('touchstart',unlock);}};window.addEventListener('click',unlock,{{once:true}});window.addEventListener('keydown',unlock,{{once:true}});window.addEventListener('touchstart',unlock,{{once:true}});}});}}}}
...
audio.ontimeupdate=()=>{{const dur=getTrackDuration(),cur=seekOffset+(isFinite(audio.currentTime)?audio.currentTime:0);if(dur>0){{progressBar.style.width=Math.min(100,Math.max(0,(cur/dur)*100))+'%';durationEl.textContent=formatTime(dur);}}currentTimeEl.textContent=formatTime(cur);}};
audio.onloadedmetadata=()=>{{durationEl.textContent=formatTime(getTrackDuration());}};
...
prevBtn.onclick=()=>{{const cur=seekOffset+(isFinite(audio.currentTime)?audio.currentTime:0);if(cur>3)setTrack(currentIndex,!audio.paused,0);else if(currentIndex>0)setTrack(currentIndex-1,true,0);}};
...
progressContainer.onclick=e=>{{const dur=getTrackDuration();if(!dur||dur<=0)return;const rect=progressContainer.getBoundingClientRect(),pos=Math.min(1,Math.max(0,(e.clientX-rect.left)/rect.width));setTrack(currentIndex,!audio.paused,pos*dur);}};
loadData();
</script></body></html>"""
...
function convertUrl(v){{v=v.trim();if(!v)return'';const h=v.match(/#\\/?(tracks|albums|playlists|artists|track|album|playlist|artist)\\/(.+)$/i);if(h){{let p=h[2].split('/'),t=h[1].toLowerCase().replace(/s$/,'');return`${{BASE_URL}}/s/${{t}}/${{p[0]}}/${{p.slice(1).join('/')}}`;}}const m=v.match(/\\b(tracks|albums|playlists|artists|track|album|playlist|artist)\\/(.+)$/i);if(m&&!v.includes('/s/')){{let p=m[2].split('/'),t=m[1].toLowerCase().replace(/s$/,'');return`${{BASE_URL}}/s/${{t}}/${{p[0]}}/${{p.slice(1).join('/')}}`;}}return v;}}
