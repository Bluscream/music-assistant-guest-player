...
def render_player_page(
    ...
    player_css = (
        ...
        ".main-content{position:relative;z-index:1;flex:1;display:flex;padding:32px 48px 16px;gap:48px;overflow:hidden;transition:all 0.3s ease;}"
        ".main-content.single-item{justify-content:center;align-items:center;padding:24px 32px 16px;gap:0;}"
        ".now-playing-panel{flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;text-align:center;max-width:50%;transition:all 0.3s ease;}"
        ".main-content.single-item .now-playing-panel{max-width:min(500px,90vw);width:100%;margin:0 auto;}"
        ".cover-container{width:min(380px,60vh);aspect-ratio:1/1;border-radius:16px;overflow:hidden;box-shadow:0 20px 50px rgba(0,0,0,0.7);background:#111;margin-bottom:24px;border:1px solid var(--border);transition:all 0.3s ease;}"
        ".main-content.single-item .cover-container{width:min(420px,50vh);margin-bottom:20px;}"
        ...
        ".main-content.single-item .now-playing-meta{max-width:100%;}"
        ...
        ".tracklist-panel{flex:1;display:flex;flex-direction:column;background:var(--panel-bg);backdrop-filter:blur(20px);-webkit-backdrop-filter:blur(20px);border-radius:16px;border:1px solid var(--border);padding:20px;overflow:hidden;transition:opacity 0.3s ease;}"
        ".main-content.single-item .tracklist-panel{display:none !important;}"
...
trackCountEl.textContent=`${{playlist.length}}`;renderTracklist();setTrack(getInitialTrackIndex(),AUTO_PLAY);}}
window.addEventListener('hashchange',()=>{{const idx=getInitialTrackIndex();if(idx!==currentIndex)setTrack(idx,!audio.paused);}});
function renderTracklist(){{trackListEl.innerHTML='';const frag=document.createDocumentFragment();playlist.forEach((t,idx)=>{{const row=document.createElement('div');row.className=`track-row ${{idx===currentIndex?'active':''}}`;const img=t.image||'{escaped_img}';const durLabel=(t.duration&&t.duration>0)?formatTime(t.duration):'LIVE';row.innerHTML=`<img src="${{img}}" class="track-row-art" alt="" loading="lazy"><div class="track-row-info"><span class="track-row-title">${{t.name}}</span><span class="track-row-artist">${{t.artist||'{escaped_site_name}'}}</span></div><span class="track-row-dur">${{durLabel}}</span>`;row.onclick=()=>setTrack(idx,true);frag.appendChild(row);}});trackListEl.appendChild(frag);}}
function setTrack(idx,play=true,offset=0){{if(idx<0||idx>=playlist.length)return;const prevIdx=currentIndex;currentIndex=idx;seekOffset=offset;const t=playlist[idx];trackTitleEl.textContent=t.name;trackArtistEl.textContent=t.artist||'{escaped_site_name}';const img=t.image||'{escaped_img}';artImg.src=img;backdrop.style.backgroundImage=`url('${{img}}')`;let base=t.stream_url.startsWith('http')?t.stream_url:window.location.origin+t.stream_url;const join=base.includes('?')?'&':'?';audio.src=`${{base}}${{join}}offset=${{Math.floor(offset)}}&_t=${{Date.now()}}`;audio.load();if(playlist.length>1)history.replaceState(null,'',`#${{t.id||(idx+1)}}`);const isLive=isLiveStream(),dur=getTrackDuration();durationEl.textContent=isLive?'LIVE':formatTime(dur);durationEl.style.color=isLive?'var(--accent)':'var(--text-muted)';durationEl.style.fontWeight=isLive?'700':'400';progressBar.style.width=isLive?'100%':((dur>0?Math.min(100,Math.max(0,(offset/dur)*100)):0)+'%');progressContainer.style.cursor=isLive?'default':'pointer';currentTimeEl.textContent=formatTime(offset);if(trackListEl.children.length>0){{if(trackListEl.children[prevIdx])trackListEl.children[prevIdx].classList.remove('active');if(trackListEl.children[idx]){{trackListEl.children[idx].classList.add('active');trackListEl.children[idx].scrollIntoView({{behavior:'smooth',block:'nearest'}});}}}}if('mediaSession'in navigator)navigator.mediaSession.metadata=new MediaMetadata({{title:t.name,artist:t.artist,artwork:[{{src:img}}]}});if(play){{audio.play().catch(e=>{{console.log('Autoplay prevented:',e);const unlock=()=>{{audio.play().catch(()=>{{}});window.removeEventListener('click',unlock);window.removeEventListener('keydown',unlock);window.removeEventListener('touchstart',unlock);}};window.addEventListener('click',unlock,{{once:true}});window.addEventListener('keydown',unlock,{{once:true}});window.addEventListener('touchstart',unlock,{{once:true}});}});}}}}
